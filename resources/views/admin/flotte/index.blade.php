...
@section('content')
    ...
    <div class="card shadow-lg border-0 rounded-3 mt-4">
        <div class="card-header bg-primary text-white fw-bold">
            <i class="fas fa-truck me-1"></i> État actuel de tous les camions
        </div>
        <div class="card-body">
            @if ($camions->isEmpty())
                    ...
                    <table class="table table-hover table-bordered align-middle shadow-sm rounded">
                        ...
                        <tbody class="text-center">
                            @foreach ($camions as $camion)
                                <tr>
                                    <td class="fw-bold">{{ $camion->numero_camion }}</td>
                                    <td>{{ $camion->matriculle }}</td>
                                    <td>
                                        @if (! empty($camion->destination_location))
                                            <span class="badge bg-info">En location</span>
                                            <br><small class="text-muted">Vers {{ $camion->destination_location }}<br>Jusqu'au {{ \Illuminate\Support\Carbon::parse($camion->retour_prevu)->format('d/m/Y') }}</small>
                                        @elseif (! empty($camion->nb_colis_en_cours) && $camion->nb_colis_en_cours > 0)
                                            <span class="badge bg-primary">Envoi de colis en cours</span>
                                            <br><small class="text-muted">Depuis le {{ \Illuminate\Support\Carbon::parse($camion->date_envoi_colis)->format('d/m/Y H:i') }}</small>
                                        @elseif (($camion->actif ?? 'on') === 'on')
